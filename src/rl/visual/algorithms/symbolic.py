...
# Detección de atasco: el bot empuja contra un bloque y no avanza (move_forward
# _jump no basta para un obstáculo de 2+ de alto).
STUCK_MOVE_EPS    = 0.05  # bloques/step; por debajo, no hubo desplazamiento real
STUCK_PATIENCE    = 2     # intentos de avance seguidos sin moverse → atasco
MINE_THROUGH_DIST = 4.5   # si el árbol está a <= esto, el bloque de delante puede
                          # ser el propio tronco objetivo → minar a través
...
class SymbolicPolicy:
    ...
    def __init__(self, seed: int | None = None):
        ...
        # Memoria entre llamadas para la detección de atasco.
        self._stuck      = 0      # nº de intentos de avance sin desplazamiento
        self._turn_dir   = None   # dirección de giro fijada mientras se rodea
        self._turn_count = 0
    ...
    def act(self, obs) -> tuple[int, str]:
        ...
        dx           = _denorm(float(cur[_SIDX["dx"]]), "dx")
        dz           = _denorm(float(cur[_SIDX["dz"]]), "dz")

        # ¿Hubo desplazamiento en el último step? dx/dz ~ 0 con intención de
        # avanzar -> bloqueado. Al moverse, se reinicia la maniobra de rodeo.
        moved = (abs(dx) + abs(dz)) > STUCK_MOVE_EPS
        if moved:
            self._stuck    = 0
            self._turn_dir = None
        ...
        # 2. Árbol visible y lejos → avanzar (o desatascar si no se progresa)
        if tree_vis and tree_dist > CLOSE_DISTANCE:
            if not moved:
                self._stuck += 1
            if self._stuck >= STUCK_PATIENCE:
                return self._unstuck(tree_vis, tree_dist)
            action = ("move_forward_jump"
                      if self._rng.random() < 0.5 else "move_forward_sprint")
    ...
    def _unstuck(self, tree_vis: bool, tree_dist: float) -> tuple[int, str]:
        """
        Reacción al atasco: un bloque delante impide avanzar y move_forward_jump
        no lo supera (obstáculo de 2+ de alto).

        Híbrido:
          - Si el árbol está al alcance (<= MINE_THROUGH_DIST), el bloque de
            delante suele ser el propio tronco objetivo → minar a través (attack).
          - Si no, reorientar: mantener un giro fijo varios steps (para cambiar de
            rumbo de verdad) e intercalar un salto-avance para rodear el obstáculo.
        """
        if tree_vis and tree_dist <= MINE_THROUGH_DIST:
            return _AIDX["attack"], "unstuck_mine"

        if self._turn_dir is None:
            self._turn_dir   = self._rng.choice(["camera_left", "camera_right"])
            self._turn_count = 0
        self._turn_count += 1

        # Cada 3 steps, un salto-avance en el nuevo rumbo; el resto, girar.
        if self._turn_count % 3 == 0:
            return _AIDX["move_forward_jump"], "unstuck_jump"
        return _AIDX[self._turn_dir], "unstuck_turn"
