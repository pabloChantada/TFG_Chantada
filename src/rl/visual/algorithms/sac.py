...
class ReplayBuffer:
    ...
    def push(self, img, state, action, reward, next_img, next_state, done):
        # Las imágenes se guardan en uint8 [0,255] (x4 menos RAM que float32);
        # se normalizan a [0,1] al muestrear, ya en GPU.
        self._buf.append((
            np.asarray(img,        dtype=np.uint8),
            np.asarray(state,      dtype=np.float32),
            int(action),
            float(reward),
            np.asarray(next_img,   dtype=np.uint8),
    ...
    def sample(self, batch_size: int):
        ...
        return (
            torch.tensor(np.array(imgs),        dtype=torch.uint8),
            torch.tensor(np.array(states),      dtype=torch.float32),
            torch.tensor(actions,               dtype=torch.long),
            torch.tensor(rewards,               dtype=torch.float32),
            torch.tensor(np.array(next_imgs),   dtype=torch.uint8),
...
class DiscreteSACAgent:
    ...
    def step(self, obs: dict, action: int, reward: float,
        ...
        # Se guarda la imagen cruda (uint8 [0,255]); la normalización a [0,1] la
        # hace _train_batch al muestrear, para no cuadruplicar la RAM del buffer.
        self.buffer.push(obs["image"], obs["state"], action, train_reward,
                         next_obs["image"], next_obs["state"], done)
        self._step += 1
    ...
    def _train_batch(self) -> dict:
        ...
        next_imgs   = next_imgs.to(self.device).float() / 255.0
