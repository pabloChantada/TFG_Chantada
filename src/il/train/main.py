...
LSTM_HIDDEN = 256
# Dimensión a la que se proyecta el estado en la variante solo-estado (sin visión)
STATE_ONLY_FEAT_DIM = 256
...
def parse_args():
    ...
    parser.add_argument('--model',       type=str,   default='rnn',
                        choices=['rnn', 'convlstm', 'vit', 'state'],
                        help='Arquitectura del modelo: rnn (GRU+LSTM), convlstm (CNN+ConvLSTM), '
                             'vit (ViT+LSTM) o state (solo-estado, sin visión)')
...
def print_run_summary(args, run_timestamp, extractor, model, dataset,
    ...
    if extractor is not None:
        ...
        print(f"  Arquitectura : Solo-estado (sin visión)")
        print(f"  state_proj   : Linear({STATE_DIM} → {model.feat_dim})")
        print(f"  LSTM         : ({model.feat_dim} → {model.lstm_hidden})")
        print(f"  action_head  : Dropout(0.5) → Linear({model.lstm_hidden} → {len(id2pair)})")
        print(f"  camera_head  : Dropout(0.3) → Linear({model.lstm_hidden} → {model.camera_dim}) → Tanh")
...
def train_epoch(extractor, model, loader, criterion, camera_criterion,
    ...
    for imgs, states, labels, camera_targets in loader:
        ...
        if extractor is not None:
            ...
            # Variante solo-estado: sin visión, el modelo opera sobre el estado.
            logits, camera_pred = model(None, states)
...
if __name__ == "__main__":
    ...
    if args.model == 'state':
        # Variante solo-estado: sin extractor visual, el tronco opera únicamente
        # sobre el vector de estado proyectado (cota inferior de la iteración).
        extractor = None
        model = MinecraftILModel(
            num_actions=num_actions,
            feat_dim=STATE_ONLY_FEAT_DIM,
            state_dim=STATE_DIM,
            lstm_hidden=LSTM_HIDDEN,
            camera_dim=CAMERA_DIM,
        ).to(device)
        model_path = os.path.join(run_dir, f"model_state_{run_timestamp}.pth")
    ...
    save_run_config(run_dir, args.model, HIDDEN_SIZE,
                    extractor.feat_dim if extractor else getattr(model, 'feat_dim', 0),
                    num_actions, variant=args.variant)
